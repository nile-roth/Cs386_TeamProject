...
User inputs wage
Basic Flow:
The use case begins when an employee opens the application.
The employee inputs their hourly wage 
The employee selects ‘start shift’ and timer starts
Alternate Flows:
If a user doesn’t input a valid wage, an error message will be displayed
The user must return to step iii of basic flow
Subflows:
NA
Key Scenarios:
Employee is starting a shift
Post-Conditions:
...
Preconditions:
User inputs wage
User starts a shift clock
Basic Flow of Events:
The use case begins when the user opens the application
The user enters their wage
The user begins their shift
The user pauses the clock by selecting the corresponding button.
Alternative Flows: 
If in step 1, the user does not input their wage, then
An error message will appear and the user cannot progress
After they input their wage, the use case resumes at  step iii
If in step iv, the user does not pause the clock
Timer will progress
Subflow:
NA
Key Scenarios:
...
Post-conditions:
The timer and wage clock will stop until the user resumes the clock
Special Requirements:
...
Actor:
A college student who works part-time
...
Basic flow of Events: 
The use case begins when the user opens the application
The user begins their shift and inputs how much they get paid per hour
At the end of their shift, they select end shift and view earnings
The user is prompted to click a button after viewing their earnings to make sure the wage on the screen is correct. 

Alternative Flows:
If in step three, the viewer does not select view earnings.
1.The user is prompted through a button to see if they want to view their current earnings 
2. If they choose yes, their current earnings are displayed. 
The use case resumes at the step where the user is asked if they want to end the shift. 

Subflow:
would not require any subflows

Key scenarios:
The user ends their shifts and wants to confirm their pay.

Post Conditions:
The user is asked to reset the timer, so they are able to track their hours and pay again.
...
A retail employee
Preconditions:
i. 	User needs to know hourly wage
Basic Flow of Events:
i.	The use case begins when the user opens the application
ii. 	The user then accesses the input prompt
iii. 	The user inputs their hourly wage and confirms
iiii. 	The use case ends.
Alternative Flow of Events
If in step 3 of the basic flow the user input their incorrect wage, then
i. 	The user would select an options button
ii. 	The user would access the wage input prompt again
...
Wage should be able to be re-entered in case of a mistake, or change in wage.
 ...
 Worker ends shift clock: The worker stops the shift clock to end the timer and stop the wage count
Actor: Worker
Preconditions:
...
The application confirms that the user is sure they would like to end their shift
The use case ends.
...
Subflows: Step 4
The application prompts the user to confirm they would like to end their shift
The user answers whether or not they would like to permanently stop the clock
...
The application asks the user to confirm whether they want to end their shift
...
Veronica - 
As a teenager who works at DoorDash, I want to track my hours and my current pay so that I can see how many more hours I should work to reach a certain amount of money.
          
Priority: Must
          
Estimate: 3 hours

As a college student that works at a fast food restaurant, I want to track how much money I’ve made in a couple days of work so that I make sure I am not being shorted on my pay.
...
Nile-
As a HM employee, I want to view how much I’ve made in a shift so that I can clock out  when I hit 70$.
...
Rain-
As an employee who works without an ADP system, I want a convenient way of tracking my hours for my shift.
...
Rebecca-
As a high school student with my first job, I want to see how much money I make in a shift to learn how to save and budget. 
...
Regan
As a part-time cashier, I want a reliable way to view my wages for a shift so I do not get underpaid.
          
Priority: Must
          
Estimate: 3 hours

As a server who picks up shifts for my coworkers, I want a product to store my weekly hours so I don’t lose track.
